...
matrix_zones_wifi = [[6.124, -75.946, 1035],
                     [6.125, -75.966, 109], [6.135, -75.976, 31], [6.144, -75.836, 151]]  # Se quito las cordenas [6.144, -75.836, 151] porque estan fuera del rango
matrix_coordinates = []
...
def flag_coordinates(coor_value, limin_inf, limin_sup):
    #print("llegada : ", coor_value, " Limite inf ",limin_inf, " limin_sup", limin_sup)
    if coor_value >= limin_inf and coor_value <= limin_sup:
        return True
    else:
        return False


def validates_matrix(matrix):
    matrix_valid = []
    for row in matrix:
        lon = row[1]
        lat = row[0]
        #print("Longuitud : ", lon, type(lon))
        #print("Latitud : ", lat, type(lat))
        flag_lon = flag_coordinates(lon, longitud_inf, longitud_sup)
        flag_lat = flag_coordinates(lat, latitud_inf, latitud_sup)
        #print("flag longitud : ", flag_lon)
        #print("flag latitud : ", flag_lat)
        if flag_lat and flag_lon:
            matrix_valid.append(row)
...
def print_travel(point_info, time_car, time_bike):
...
session_start()
matrix_zones_wifi = validates_matrix(matrix_zones_wifi)
...
while iterator != 7:
   ...
   # print("El contador esta :", count_sessions)
    ...
    if iterator == 3:
        if start_coordinated:
            ...
            if locate < 1 or locate > 3:
                ...
                current_data = [matrix_coordinates[locate-1]
                                [0], matrix_coordinates[locate-1][1]]
                ...
                zone_wifi_1 = [matrix_neartest_point[0]
                               [0], matrix_neartest_point[0][1], matrix_neartest_point[0][3]]
                ...
                if option_entry == 1 or option_entry == 2:
                    ...
                    point_info = matrix_neartest_point[index]
                    time_car, time_bike = calculate_time_travel_point(
                        point_info)
                    info_point = [matrix_neartest_point[index]
                                  [0], "Carro", time_car]
                    print_travel(point_info, time_car, time_bike)
                    flag_location = False
    ...
    if iterator == 4:
        if start_coordinated or flag_location:
            print_exit("Error de alistamiento")
        else:
            data_key = ['atual', 'zonawifi1', 'recorrido']
            data_values = [current_data, zone_wifi_1, info_point]
            data = dict(zip(data_key, data_values))
            print(data)
            print("¿Está de acuerdo con la información a exportar? Presione 1 para confirmar, 0 para regresar al menú principal")
            export = int(input())
            if export == 1:
                print_exit("Exportando archivo")
            if export == 0:
                continue

    if iterator == 5:
        path = "./data.csv"
        archive = pd.read_csv(path, encoding="utf-8")
        matrix_zones_wifi = archive.values.tolist()
        #print("Archivo ", matrix_zones_wifi)
        matrix_zones_wifi = validates_matrix(matrix_zones_wifi)
        # print(matrix_zones_wifi)
